...
function parseSchema(obj) {
    ...
    if (obj.applications) {
        ...
        if (obj.applications.length === 1) {
            let type = obj.applications[0].name;
            if (type instanceof String ) type = type.split('$')[0]
        ...
        for (let i in obj.applications) {
            let type = obj.applications[i].name;
            if (type instanceof String ) type = type.split('$')[0]
            if (type == 'object' || type == 'string' || type == 'integer' || type == 'boolean') {
                oneOf.push({
                    "type": type
    ...
    for (let i = 1; i < tags.length; i++) {
        if (tags[i].title == 'property') {
            ...
            if (schema) {
                ...
                const type = parseType(tags[i].type);
                const format = parseFormat(tags[i].type);
            ...
            for (let j in comments[i]) {
                ...
                if (title == 'param') {
                    ...
                    if (!schema) {
                        ...
                        properties.format = parseFormat(comments[i][j]['type'])
